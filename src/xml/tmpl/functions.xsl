...
<!-- ハンガリアンに変換(1ワード) -->
<xsl:template name="SingleHungarian">
    <xsl:param name="s"/>
        ...
        <xsl:when test="string-length($after) > 0 or string-length($before) > 0">
            <xsl:if test="string-length($before) > 0">
                <xsl:if test="not($lower)">
                    <xsl:call-template name="SingleHungarian"><xsl:with-param name="s" select="$before"/></xsl:call-template>
                </xsl:if>
                <xsl:if test="$lower">
            ...
            <xsl:if test="string-length($after) > 0">
              <xsl:call-template name="Hungarian"><xsl:with-param name="s" select="$after" /></xsl:call-template>
            </xsl:if>
        </xsl:when>

        <xsl:when test="string-length($s) > 0 and not(contains($s, '_') )">
            <xsl:if test="not($lower)">
                <xsl:call-template name="SingleHungarian"><xsl:with-param name="s" select="$s" /></xsl:call-template>
            </xsl:if>
            <xsl:if test="$lower">
